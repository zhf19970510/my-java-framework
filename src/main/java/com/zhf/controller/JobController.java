...
import com.zhf.entity.LayuiData;
...
public class JobController {
	...
	public int addjob(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		bean.setJobStatus(CommonConstant.JOB_STATE.YES);
		try {
	...
	public int changeState(@RequestParam(value = "id") String idStr)throws Exception{
		...
		updateBean.setId(id);
		//如果是现在是启用，则停用
		if(CommonConstant.JOB_STATE.YES == result.getJobStatus()){
			updateBean.setJobStatus(CommonConstant.JOB_STATE.NO);
		...
		//如果现在是停用，则启用
		if(CommonConstant.JOB_STATE.NO == result.getJobStatus()){
			updateBean.setJobStatus(CommonConstant.JOB_STATE.YES);
	...
	public int updateByBean(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if( result.getJobStatus() ==CommonConstant.JOB_STATE.YES ){
